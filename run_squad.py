...
def read_squad_examples_for_zh(input_file, is_training):
  """Read a SQuAD json file into a list of SquadExample."""
  with tf.gfile.Open(input_file, "r") as reader:
    input_data = json.load(reader)["data"]
        ...
        if is_training:
          ...
          if not is_impossible:
            ...
            if ori_start_position == -1:
                ...
                continue
            answer_length = len(cleaned_answer_text)
            start_position = char_to_word_offset[ori_start_position]
            end_position = char_to_word_offset[ori_start_position + answer_length - 1]
            # Only add answers where the text can be exactly recovered from the
            # document. If this CAN'T happen it's likely due to weird Unicode
            # stuff so we will just skip the example.
            #
...
def main(_):
  ...
  if FLAGS.do_train:
    train_examples = read_squad_examples_for_zh(
  ...
  if FLAGS.do_predict:
    eval_examples = read_squad_examples_for_zh(
